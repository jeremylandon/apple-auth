...
namespace Golap.AppleAuth.Tests
{
    public class AppleVerifierTest
    {
        ...
        {
            var expected =
                $"{{\"alg\":\"RS256\",\"typ\":\"JWT\",\"kid\":\"abc\"}}.{{\"iss\":\"https://appleid.apple.com\",\"aud\":\"{Audience}\",\"exp\":2147483647,\"iat\":333825230,\"sub\":\"000000.xxxx.1111\",\"c_hash\":\"xxxx\",\"email\":\"xxxx@privaterelay.appleid.com\",\"email_verified\":\"true\",\"is_private_email\":\"true\",\"auth_time\":333825230,\"nonce_supported\":true}}";

            var appleResponse = new AppleKeysResponse { Keys = new[] { new JsonWebKey(AppleJsonKey) } };
        ...
        public async Task ValidateAsync_NotValidSignature_ThrowSecurityTokenInvalidSignatureException()
        {
            ...
            await FluentActions.Invoking(() => verifier.ValidateAsync(jwtToken, Audience)).Should().ThrowAsync<SecurityTokenInvalidSignatureException>();
        } 
        ...
        public async Task ValidateAsync_AppleDoenstReturnKid_ThrowAppleAuthException()
        {
            ...
            await FluentActions.Invoking(() => verifier.ValidateAsync(jwtToken, Audience)).Should().ThrowAsync<AppleAuthException>();
        }
        ...
        public async Task ValidateAsync_AppleReturnError_ThrowAppleAuthException()
        {
            ...
            await FluentActions.Invoking(() => verifier.ValidateAsync(jwtToken, Audience)).Should().ThrowAsync<AppleAuthException>().WithMessage(responseData);
        }
